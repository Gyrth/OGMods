...
const float WALK_THRESHOLD = 0.1;
...
class Animation {
	...
	void Stop(){
		for(uint i = 0; i < frame_objects.size(); i++){
			frame_objects[i].SetEnabled(false);
		}
	}

	void Start(){
		for(uint i = 0; i < frame_objects.size(); i++){
			frame_objects[i].SetEnabled(true);
		}
	}
...
Animation @attack_down_animation;
Animation @attack_right_animation;
...
void UpdateSpritePosition(){
	if(@current_animation != null){
		...
		current_animation.current_frame.SetRotation(quaternion());
	}else{
		Log(warning, "Problem");
	}
}
	...
	if(UpdateDirection() || state_changed){
		switch(direction){
			case Up:
				...
				break;
			case Down:
				SetAnimation(@attack_down_animation);
				break;
			case Right:
				SetAnimation(@attack_right_animation);
				break;
			case Left:
				SetAnimation(@attack_left_animation);
				break;
		}
	}
...
void UpdateIdle(){
	if(UpdateDirection() || state_changed){
		...
		switch(direction){
			case Up:
				SetAnimation(@idle_up_animation);
				break;
			case Down:
				SetAnimation(@idle_down_animation);
				break;
			case Right:
				SetAnimation(@idle_right_animation);
				break;
			case Left:
				SetAnimation(@idle_left_animation);
				break;
		}
	}
	...
	if(UpdateDirection() || state_changed){
		switch(direction){
			case Up:
				...
				break;
			case Down:
				SetAnimation(@walk_down_animation);
				break;
			case Right:
				SetAnimation(@walk_right_animation);
				break;
			case Left:
				SetAnimation(@walk_left_animation);
				break;
		}
	}

	float vertical_direction = max(-1.0f, min(1.0f, dot(vec3(0.0f, 0.0f, 1.0f), this_mo.velocity)));
	float horizontal_direction = max(-1.0f, min(1.0f, dot(vec3(1.0f, 0.0f, 0.0f), this_mo.velocity)));
...
void PostInit(){
	walk_down_animation.Initialize();
	walk_right_animation.Initialize();
	walk_left_animation.Initialize();
	walk_up_animation.Initialize();

	idle_down_animation.Initialize();
	idle_right_animation.Initialize();
	idle_left_animation.Initialize();
	idle_up_animation.Initialize();

	attack_down_animation.Initialize();
	attack_right_animation.Initialize();
...
void Reset() {
	...
	walk_down_animation.Reset();
	walk_right_animation.Reset();
	walk_left_animation.Reset();
	walk_up_animation.Reset();

	idle_down_animation.Reset();
	idle_left_animation.Reset();
	idle_right_animation.Reset();
	idle_up_animation.Reset();

	attack_down_animation.Reset();
	attack_left_animation.Reset();
