...
enum movement_states {idle, walk, jump};
...
class Animation {
	...
	void Initialize(){
		for(uint i = 0; i < frame_paths.size(); i++){
			Log(warning, frame_paths[i]);
			int frame_id = CreateObject(frame_paths[i], true);
			Log(warning, "Object id " + frame_id);
...
Animation @walk_animation = Animation({	"Data/Objects/2d_character_walk_1.xml",
...
Animation @idle_animation = Animation({	"Data/Objects/2d_character_idle.xml"});
Animation @jump_animation = Animation({	"Data/Objects/2d_character_jump.xml"});
...
void UpdateMovement(){
	...
	vec3 target_velocity = GetTargetVelocity();
	/* Log(warning, "target_velocity " + length(target_velocity)); */
