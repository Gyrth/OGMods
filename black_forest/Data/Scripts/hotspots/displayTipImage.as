...
vec4 g_tint;
int dice = rand()%56;
string image_path = "/Data/UI/Icons/tip"+dice+".png";
...
void SetParameters() {
   // level.SendMessage("displaytext \""+"set parameters is"+image_path+" and dice is at "+dice);
    params.AddFloatSlider("[1] Scale", 0.3, "min:0.0,max:1.0,step:0.1,text_mult:100");
    g_scale = params.GetFloat("[1] Scale");

    params.AddIntSlider("[2a] Red Tint", 120, "min:0.0,max:255.0");
    params.AddIntSlider("[2b] Green Tint", 120, "min:0.0,max:255.0");
    params.AddIntSlider("[2c] Blue Tint", 120, "min:0.0,max:255.0");
    params.AddIntSlider("[2d] Alpha Tint", 0.0, "min:0.0,max:255.0");
...
void Draw() {
    if(g_is_active) {
        ...
        //level.SendMessage("displaytext \""+"image path is "+image_path+"\"");
        ...
        image.scale = vec3(image_scale*0.7, image_scale*0.7, 0.0f);
        image.position = vec3(image_pos.x*1.4, image_pos.y*0.15, 0.0f);
       	//image.color = g_tint;
    }
}
