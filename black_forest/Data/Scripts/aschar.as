...
// COMPATIBILITY ISSUE
// UNDERGROWTH CHANGES HAS BEEN COMMENT WITH undergrowth SO IT IS EASY TO TRACK THEM DOWN AND PORT THEM TO THE NEW ASCHAR
enum WalkDir {
    WALK_BACKWARDS,
    STRAFE,
    FORWARDS
...
void HitByItem(string material, vec3 point, int id, int type) {
    ...
    //undergrowth added spearz to the condition
    if(species != _dog || io.GetLabel() == "spear" || io.GetLabel() == "spearz" || io.GetLabel() == "big_sword"){
        if(type == 2) {
...
int HitByAttack(const vec3 &in dir, const vec3 &in pos, int attacker_id, float attack_damage_mult, float attack_knockback_mult) {
    ...
    if(!can_passive_block) {
        ...
        if(enemy_primary_weapon_id != -1) {
            ItemObject@ weap = ReadItemID(enemy_primary_weapon_id);
            //undergrowth added spearz for item behavior
            if(weap.GetLabel() == "spear" || weap.GetLabel() == "spearz" ){
...
void UpdateThreatAmount(const Timestep &in ts) {
    if((state != _movement_state && state != _attack_state) || (dialogue_stop_time > time - 3.0 && !NeedsCombatPose())) {
        ...
        if(!WantsToAttack() && WantsToDragBody() && !NeedsCombatPose()) {
            ...
            //undergrowth added spearz for behavior
            if(weap_label != "big_sword" && weap_label != "spear" && weap_label != "spearz" && weap_label != "staff"){
...
void UpdateAttacking(const Timestep &in ts) {
    ...
    if(weapon_id != -1) {
        ...
        //undergrowth change added spearz as tag since we need custom tag for sheathability
        if(io.GetLabel() == "spear" || io.GetLabel() == "spearz"){
...
void ThrowWeapon(int slot) {
    if(weapon_slots[slot] != -1) {
        ...
        // if(target != -1) {
            ...
            //undergrowth change for spearz to behave like a spear
            if((io.GetLabel() == "spear" || io.GetLabel() == "staff" || io.GetLabel() == "spearz") && throw_anim){
                effective_mass *= 0.25f;
            }
            ...
            //undergrowth change for spearz to behave like a spear
            if((io.GetLabel() == "spear" || io.GetLabel() == "spearz" || io.GetLabel() == "staff") && throw_anim){
...
int GetThrowTarget() {
    ...
    //undergrowth change for spearz to act like a spear
    if(weapon_slots[primary_weapon_slot] != -1 && (ReadItemID(weapon_slots[primary_weapon_slot]).GetLabel() == "spear"|| ReadItemID(weapon_slots[primary_weapon_slot]).GetLabel() == "spearz")){
    ...
    if(going_to_throw_item && going_to_throw_item_time <= time && going_to_throw_item_time > time - 1.0f) {
        if(!flip_info.IsFlipping() || flip_info.flip_progress > 0.5f) {
            ...
            if(on_ground && !flip_info.IsFlipping() && primary_weapon_id != -1 && (label == "big_sword" || label == "spear" || label == "staff")) {
                ...
                //undergrowth change added spearz to behave like a spear
                } else if(label == "spear" || label == "spearz" || label == "staff"){
                ...
                attacking_with_throw = 0;
                throw_anim = true;
                //undergrowth added cheevo so shuriken jitsu be compiled (they didnt added it in the patch 1.3 :( my only request) 
                if(this_mo.controlled){
