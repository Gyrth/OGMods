...
string target_animation = "Data/Animations/gun_magazine_insert.anm";
...
void Update(int num_frames) {
	...
	if(this_mo.controlled && knocked_out != _dead){
		UpdateControls();
		UpdateMovement();
		UpdateAiming();
		UpdateFootsteps();

		SetSoundPosition(drone_sound_id, this_mo.position);
		SetSoundPitch(drone_sound_id, length(this_mo.velocity) / 10.0);
		SetSoundGain(drone_sound_id, 0.5f);
	}else{
		if(GetInputPressed(0, "attack")){
			level.SendMessage("show_red false");
			ResetLevel();
		}
	}
...
void UpdateFootsteps(){
	...
	if(step_counter > step_threshold){
		...
		switch(rand() % 5) {
			case 0:
				path = "Data/Sounds/receiver_gun_fs_1.wav"; break;
			case 1:
				path = "Data/Sounds/receiver_gun_fs_2.wav"; break;
			case 2:
				path = "Data/Sounds/receiver_gun_fs_3.wav"; break;
			case 3:
				path = "Data/Sounds/receiver_gun_fs_4.wav"; break;
			default:
				path = "Data/Sounds/receiver_gun_fs_5.wav"; break;
		}
		PlaySound(path, this_mo.position + vec3(0.0, -1.0, 0.0));
	}
}
...
void UpdateMovement(){
	...
	if(length(target_velocity) > 0.01){
		/* DebugDrawLine(this_mo.position, this_mo.position + target_velocity, vec3(1.0, 0.0, 0.0), _delete_on_update); */
		MoveTowards(this_mo.position + target_velocity);
	}

	vec3 current_facing = this_mo.GetFacing();
	/* this_mo.SetRotationFromFacing(mix(current_facing, target_facing, time_step * 10.0)); */
...
vec3 GetTargetVelocity() {
	...
	{
		right = camera.GetFacing();
		float side = right.x;
		right.x = -right .z;
		right.z = side;
	}

	vec3 up_direction = QuaternionFromMat4(this_mo.rigged_object().skeleton().GetBoneTransform(0)) * vec3(0.0, 0.0, 1.0);
	/* DebugDrawLine(this_mo.position, this_mo.position + up_direction, vec3(0.0, 1.0, 0.0), _delete_on_update); */

	float movement_speed = 3.0f;

	target_velocity += GetMoveXAxis(this_mo.controller_id) * right * time_step * movement_speed;
	target_velocity -= GetMoveYAxis(this_mo.controller_id) * camera.GetFacing() * time_step * movement_speed;
	target_facing = normalize(target_velocity);
	target_velocity += (GetInputDown(0, "jump")?1.0f:0.0f) * up_direction * time_step * movement_speed;
	target_velocity -= (GetInputDown(0, "lctrl")?1.0f:0.0f) * up_direction * time_step * movement_speed;

	if(length_squared(target_velocity) > 1.0) {
		target_velocity = normalize(target_velocity);
	}

	/* Log(warning, "speed" + length(target_velocity)); */
	return target_velocity;
}
...
void UpdateCharacterRotation(){
	if(this_mo.controlled){
		...
		vec3 current_facing = this_mo.GetFacing();
		this_mo.SetRotationFromFacing(mix(current_facing, facing, time_step * 5.0));
	}
}
...
void UpdateCamera(const Timestep &in ts){
	...
	vec3 cam_pos = this_mo.position + vec3(0.0f, 0.01f, 0.0f);

	camera.SetFOV(current_fov);
	camera.SetPos(cam_pos);
	camera.SetDistance(GetInputDown(0, "grab")?0.25f: 1.0f);
...
void UpdateControls(){
	...
	}else if(GetInputPressed(this_mo.controller_id, "attack")){
		Shoot();
	}
}
...
void Shoot(){
	if(rounds <= 0){
		PlaySound("Data/Sounds/receiver_gun_click.wav", this_mo.position);
		return;
	}
	vec3 forward = this_mo.GetFacing();
	vec3 muzzle_offset = forward * 0.15;
	vec3 spawn_point = this_mo.position + vec3(0.0, -0.02, 0.0) + ((1.0 - aiming_amount) * (vec3(0.0, -0.25, 0.0)));
...
void Reset() {
	...
	StopSound(drone_sound_id);
}

bool Init(string character_path) {
	drone_sound_id = PlaySoundLoopAtLocation("Data/Sounds/drone.wav", this_mo.position, 0.5);
...
void FinalAnimationMatrixUpdate(int num_frames) {
	...
	local_to_world.origin = location + ((1.0 - aiming_amount) * (vec3(0.0, -0.25, 0.0)));
...
void ApplyPhysics(const Timestep &in ts) {
	...
	for(int i = 0; i < sphere_col.NumContacts(); i++){
		...
		on_ground = true;
		PlaySoundGroup("Data/Sounds/weapon_foley/impact/weapon_metal_hit_metal.xml", collision_point, 0.25);
		MakeMetalSparks(collision_point);
	...
	if(knocked_out != _dead){
		vec3 up_direction = QuaternionFromMat4(this_mo.rigged_object().skeleton().GetBoneTransform(0)) * vec3(0.0, 0.0, 1.0);
		this_mo.velocity += up_direction * (length(physics.gravity_vector)) * time_step;
	}
...
vec3 target_facing = vec3(0.0, 0.0, 0.0);
...
void MoveTowards(vec3 position){
	...
	vec3 drone_translation = this_mo.position;
	vec3 current_direction = this_mo.GetFacing();
	float movement_speed = 1.0;

	float dot_product = dot(velocity_direction, target_direction);
	vec3 direction = target_direction;
	...
	velocity += direction * movement_speed * time_step;
...
void Died(){
	...
	level.SendMessage("show_red true");
	aiming_amount = 0.0;
	knocked_out = _dead;
}
...
void PostReset(){
	drone_sound_id = PlaySoundLoopAtLocation("Data/Sounds/drone.wav", this_mo.position, 0.5);
	SetSoundGain(drone_sound_id, 0.25f);
	CacheSkeletonInfo();
}
