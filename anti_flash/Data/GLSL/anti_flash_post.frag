...
#define INV_PROJECTION_MATRIX proj_mat
...
void main(void)
{
   ...
   color *= 0.25;
    ...
    float dd = DistFromDepth(texture(tex1, tex + vec2(0., 1.) * pixel_size).r);
    float dl = DistFromDepth(texture(tex1, tex + vec2(-1., 0.) * pixel_size).r);
