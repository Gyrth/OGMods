...
*/
	...
	MovementObject@ ragdoll_char = ReadCharacterID(8);

	RiggedObject@ target_rigged_object = ragdoll_char.rigged_object();
	Skeleton@ target_skel = target_rigged_object.skeleton();

	if(!post_init_done){
		ragdoll_char.Execute("temp_health = -1.0;permanent_health = -1.0;blood_health = -1.0;SetKnockedOut(_dead);CharacterDefeated();Ragdoll(_RGDL_LIMP);");
		/* ragdoll_char.Ragdoll(); */
		target_rigged_object.DisableSleep();
		post_init_done = true;
	}

	if(ragdoll_char.GetIntVar("state") == _ragdoll_state){
		target_rigged_object.SetRagdollDamping(0.1f);
		target_rigged_object.SetRagdollStrength(0.0);

		RiggedObject@ this_rigged_object = this_mo.rigged_object();
		Skeleton@ skel = this_rigged_object.skeleton();

		array<string> bone_names = {"rightarm", "leftarm", "head", "right_leg", "left_leg", "rightear", "leftear", "torso"};

		for(int i = 0; i < skel.NumBones(); ++i) {
			if(skel.GetBoneMass(i) > 0.03) {
				mat4 mat = this_rigged_object.GetFrameMatrix(i).GetMat4() * skel.GetBindMatrix(i);
				mat.SetTranslationPart(this_mo.position + vec3(0.0f, 0.3f, 0.0f));
				vec3 bone_tip_position = mat * skel.GetPointPos(skel.GetBonePoint(i, 0));
				vec3 bone_end_position = mat * skel.GetPointPos(skel.GetBonePoint(i, 1));
				/* debug_lines.push_back(DebugDrawLine(this_mo.position, mat * skel.GetPointPos(skel.GetBonePoint(i, 1)), vec3(1.0), _fade)); */

				mat4 bone_transform = target_skel.GetBoneTransform(i);
				vec3 ragdoll_bone_position = bone_transform.GetTranslationPart();
				vec3 target_bone_position = ((bone_tip_position + bone_end_position) / 2.0f);
				float dist = distance(target_bone_position, ragdoll_bone_position);
				vec3 force_direction = normalize(target_bone_position - ragdoll_bone_position);
				vec3 force = force_direction * (100000.0f * dist);


				/* this_rigged_object.ApplyForceLineToRagdoll(force * time_step, ragdoll_bone_position, force_direction); */
				/* DebugDrawLine(bone_tip_position ,bone_end_position, vec3(1.0), _delete_on_draw); */

				for(int j = 0; j < int(bone_names.size()); j++) {
					int bone = skel.IKBoneStart(bone_names[j]);

					if(i == bone){
						/* this_rigged_object.ApplyForceToBone(force * time_step, i); */
						/* DebugDrawWireSphere(ragdoll_bone_position, 0.05f, vec3(1.0), _delete_on_draw);
						DebugDrawLine(bone_transform.GetTranslationPart(), target_bone_position, vec3(1.0, 0.0, 0.0), _delete_on_draw); */
						target_rigged_object.MoveRagdollPart(bone_names[j], target_bone_position, 1.0);
					}
				}
			}
		}
