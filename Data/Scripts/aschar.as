...
void HandleThrow() {
    ...
    if(WantsToThrowItem() && weapon_slots[primary_weapon_slot] != -1 && throw_knife_layer_id == -1){        
		...
		vec3 hit = col.GetRayCollision(transform.origin, transform.origin + axis);
		DebugDrawLine(transform.origin, hit, vec3(1.0f,1.0f,1.0f), _delete_on_update);
		...
		vec3 axis = head_rotation * vec3(0,30,20);
		vec3 hit = col.GetRayCollision(transform.origin, transform.origin + axis);
		this_mo.SetRotationFromFacing(normalize(axis));
...
void ApplyCameraControls(const Timestep &in ts) {
    ...
    if(!camera.GetAutoCamera()){
	   ...
	   if(throw_target_pos != -1){
            // Look at target
            float dist = distance(throw_target_pos, this_mo.position);
            vec3 target_facing = (throw_target_pos - this_mo.position)/dist;
            // We want more of a side view at close range, and more of a straight view in the distance
            float target_angle = max(0.2f, 1.2f / max(1.0f,dist)); 
            if(autocam.target_weight == 0.0f){
                autocam.angle = target_angle;
